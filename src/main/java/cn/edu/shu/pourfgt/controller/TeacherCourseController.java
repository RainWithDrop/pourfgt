...
public class TeacherCourseController {
    ...
    public String createCourse(String courseId, String courseName, Integer[] courseType,
                               ...
                               Double normalRatio, String[] name, Double[] ratio, Model model) throws IOException {
        ...
        List<String[]> testCourseStudent = new ArrayList<>();

        // revise student list as from file
        List<String> dataList = new ArrayList<String>();
        InputStream ips = null;
        File file1 = null;
        file1 = File.createTempFile("temp", null);
        studentList.transferTo(file1);
        ips =new FileInputStream(file1);
        file1.deleteOnExit();
        XSSFWorkbook wb = new XSSFWorkbook(ips);//XSSF支持XLSX格式表格
        XSSFSheet sheet = wb.getSheetAt(0);

        int coloumNum=sheet.getRow(0).getPhysicalNumberOfCells();
        int rowNum=sheet.getLastRowNum();//获得总行数
        for (int i = 0; i <= rowNum; i++) {
            testCourseStudent.add(
                    new String[]{
                            Integer.toString((int)sheet.getRow(i).getCell(0).getNumericCellValue()),
